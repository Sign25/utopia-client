...
_STAMINA_EXHAUSTION_ONSET = 85.0   # = neurocore MENTAL_BREAK_EXHAUSTION_FATIGUE_MIN (КОНТРАКТ-матч, не прокси: «порог=mental_break-онсет», Фрай OPEN-1 B)
...
class LocalColonyCompute:
    ...
    def _update_stamina_cost(self, cid: str) -> None:
        ...
        # интеграл стоимости за тик (fatigue над exhaustion-онсетом = реальное давление)
        self._stam_win_cost[cid] = (self._stam_win_cost.get(cid, 0.0)
                                    + max(0.0, fat - _STAMINA_EXHAUSTION_ONSET))
        ...
        logger.info("STAMINA_COST_DIAG cid=%s win=%d cost=%.1f cum=%.1f (порог-онсет=%.0f, "
                    "N=%d) — калибровка poor_win", cid, self._stam_win_n, cost,
                    self._beh_stamina_cost_cum[cid], _STAMINA_EXHAUSTION_ONSET, _STAMINA_WIN_N)
