...
class LocalColonyCompute:
    ...
    def _apply_action_fatigue(self, cid: str, action: int) -> None:
        ...
        общую apply_action_taken (re-use ACTION_FATIGUE_GLUCOSE φ-лестница, math-equiv
        с server). Закрывает GAP — client раньше НЕ копил action-fatigue (Адам не
        уставал, fat=0). glucose-дельта тоже (часть apply_action_taken). Гейт
        phi_fatigue (caller). math-equiv: те же значения, что server для system."""
        bc = self.biochem.get(cid)
        if bc is None:
            return
        try:
            from environment.biochemistry import apply_action_taken  # type: ignore
        except Exception as e:
            logger.debug("apply_action_taken import failed cid=%s: %s", cid, e)
            return
        try:
            apply_action_taken(bc, int(action))
        except Exception as e:
